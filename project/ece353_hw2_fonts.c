	...
	char character)
{
	...
	int i;
	int j;
	//calculate starting index using the ascii value of the character
	byte_index = ((((int)character) - 32) * 32) + 30;
	//set the initial position of the text
	lcd_set_pos(X_pixel, X_pixel + FONT_WIDTH - 1, Y_pixel, Y_pixel + FONT_HEIGHT - 1);
	//begin printing the character to the lcd screen
	for(i = 0; i < FONT_HEIGHT; i++){
		//get all the data for 1 row of pixels and reposition the bits
		//so it can be processed
		data = courierNewBitmap[byte_index];
		data = data << 8;
		moredata = courierNewBitmap[byte_index + 1];
		data = data + moredata;
		//read each bit in the row and see which color should be written
...
{
	int8_t initialX = X;	//save initial X pos for new line
	int i = 0;
	while(msg[i] != NULL){
		//if character will go off the screen
		if(X >= CHAR_COLUMNS){
			X = initialX;
			Y += 1;
			//skip space character if it is the first character after doing a new line
			if(msg[i] == ' '){
				i++;
				continue;
			}
		}
		//if Y pos is going off the bottom after a new line, send it to top
		if(Y >= CHAR_ROWS) Y = 0;
		lcd_print_character(X*11, Y*16, fg_color, bg_color, msg[i]);
		X += 1;
