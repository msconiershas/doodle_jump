...
static __INLINE void wireless_tx_data_payload( uint32_t data)
{
	...
	tx_data[2] = (data & 0x00FF0000) >> 16;
	tx_data[3] = (data & 0x0000FF00) >> 8;
	tx_data[4] = (data & 0x000000FF) >> 0;
	wireless_CSN_low();
...
static __INLINE void wireless_rx_data_payload( uint32_t *data)
{
	...
	temp = ((uint32_t)(rx_data[1]))<<24;
	temp |= ((uint32_t)(rx_data[2]))<<16;
	temp |= ((uint32_t)(rx_data[3]))<<8;
	temp |= ((uint32_t)(rx_data[4]))<<0;
	*data = temp;
}
